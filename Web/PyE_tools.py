...
def klassedNumberByLen(len: int):
    return round(1 + 3.22 * math.log10(len))
...
def dataRangeByValues(minValue: Decimal, maxValue: Decimal):
...
def dataAmplitudeByVariables(range: int, classesNumber: int, variationUnit: int):
    if variationUnit == 1:
        if "." in str(range / classesNumber):
            return Decimal(str(math.ceil(Decimal(str(range / classesNumber)))))
        else:
    ...
    return Decimal(str((math.ceil(Decimal(str(range / classesNumber)) / variationUnit) * variationUnit)))
